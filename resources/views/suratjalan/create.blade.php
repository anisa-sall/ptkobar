...
<head>
  ...
  <style>
    /* Semua CSS dari file asli tetap sama */
    .sidebar {
...
<body>
        ...
        <div class="modal-dialog modal-dialog-centered" style="max-width: 400px; margin: 0 auto;">
            <div class="modal-content" style="max-height: 200px; overflow: hidden;">
                ...
                <div class="modal-footer py-2" style="border-top: 1px solid #ffffffff;">
                    ...
                    <a href="{{ route('logout') }}" class="btn btn-primary btn-sm rounded-1"
    ...
    <div class="container-fluid page-body-wrapper">
      <nav class="sidebar sidebar-offcanvas sidebar-dark" id="sidebar">
        <ul class="nav">
          ...
          @endif
          @if(isset($menuAccess['stok']) && in_array($departemen, $menuAccess['stok']))
          <li class="nav-item" id="stok-menu">
          ...
          @endif
          @if(isset($menuAccess['invoice']) && in_array($departemen, $menuAccess['invoice']))
          <li class="nav-item">
            <a class="nav-link" href="#">
          ...
          @endif
          @if(isset($menuAccess['laporan']) && in_array($departemen, $menuAccess['laporan']))
          <li class="nav-item">
            <a class="nav-link" href="#">
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Form Surat Jalan Baru</h4>
                  <p class="card-description">
                    Halaman Tambah Data Surat Jalan PT. Kobar Indonesia
                  </p>
                  ...
                  <form class="forms-sample" method="POST" action="{{ route('suratjalan.store') }}" id="suratjalanForm">
                    @csrf
                    <!-- Input hidden untuk membedakan jenis submit -->
                    <input type="hidden" name="submit_type" id="submit_type" value="partial">

                    <div class="form-group">
                        <label for="nosuratjalan">No. Surat Jalan</label>
                        <input type="text" class="form-control @error('nosuratjalan') is-invalid @enderror" 
                               id="nosuratjalan" name="nosuratjalan" 
                               placeholder="Masukkan No. Surat Jalan" 
                               value="{{ old('nosuratjalan')}}" required>
                        @error('nosuratjalan')
                      ...
                      <select class="form-control @error('idcustomer') is-invalid @enderror" 
                        ...
                        @foreach ($customers as $customer)
                          <option value="{{ $customer->idcustomer }}" 
                            {{ old('idcustomer', $selected_customer) == $customer->idcustomer ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                      ...
                      <select class="form-control @error('nopo') is-invalid @enderror" 
                              id="nopo" name="nopo" 
                              {{ empty($selected_customer) ? 'disabled' : '' }} required>
                        @if (empty($selected_customer))
                          <option value="" disabled selected>Pilih customer terlebih dahulu</option>
                        @elseif (empty($pos) || count($pos) == 0)
                          <option value="" disabled selected>Tidak ada PO yang tersedia</option>
                        @else
                          <option value="" disabled selected>Pilih No. PO</option>
                          @foreach ($pos as $po)
                            <option value="{{ $po['nopo'] }}"
                              {{ old('nopo') == $po['nopo'] ? 'selected' : '' }}>
                              {{ $po['nopo'] }} - (Status: {{ $po['status'] }})
                            </option>
                          @endforeach
                        @endif
                      </select>
                      @error('nopo')
                          <div class="invalid-feedback">{{ $message }}</div>
                      @enderror
                      @if ($selected_customer && (empty($pos) || count($pos) == 0))
                        <small class="form-text text-danger fw-bold small" style="font-size: 0.7rem;">
                          <i class="mdi mdi-alert-circle-outline"></i> Tidak ada PO dengan status OPEN atau PARTIAL untuk customer ini
                        </small>
                      @elseif ($selected_customer)
                        <small class="form-text text-info small" style="font-size: 0.7rem;">
                          <i class="mdi mdi-information-outline"></i> Menampilkan PO dengan status OPEN dan PARTIAL saja
                        </small>
                      @else
                        <small class="form-text text-muted small" style="font-size: 0.7rem;">Pilih customer terlebih dahulu untuk menampilkan PO</small>
                      @endif
                    </div>

                    <div class="form-group">
                      <label for="tglpengiriman">Tanggal</label>
                      <input type="date" class="form-control @error('tglpengiriman') is-invalid @enderror" 
                             id="tglpengiriman" name="tglpengiriman" 
                             value="{{ old('tglpengiriman', request('tglpengiriman', date('Y-m-d'))) }}" required>
                      @error('tglpengiriman')
                          <div class="invalid-feedback">{{ $message }}</div>
                      @enderror
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <select class="form-control @error('nopol') is-invalid @enderror" 
                              id="nopol" name="nopol" required>
                        <option value="" disabled selected>Pilih Kendaraan</option>
                        @foreach ($kendaraans as $kendaraan)
                          <option value="{{ $kendaraan->nopol }}"
                            {{ old('nopol') == $kendaraan->nopol ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                      ...
                      <small class="form-text text-muted small" style="font-size: 0.7rem;">Petugas terdeteksi otomatis dari sistem login</small>
                    </div>
                    <button type="submit" class="btn btn-primary me-2">Submit</button>
                    <a href="{{ route('suratjalan.index') }}" class="btn btn-secondary">Cancel</a>
                  </form>
                </div>
              </div>
            </div>
  ...
  <script>
    // Fungsi untuk update PO ketika customer dipilih (menggunakan form submit partial)
    function updatePO() {
        document.getElementById('submit_type').value = 'partial';
        document.getElementById('suratjalanForm').submit();
    }

    // Aktifkan dropdown PO jika customer sudah dipilih
    document.addEventListener('DOMContentLoaded', function() {
        const customerSelect = document.getElementById('idcustomer');
        const poSelect = document.getElementById('nopo');
        
        if (customerSelect.value !== '') {
            poSelect.disabled = false;
        }
        
        // Tambahkan event listener untuk customer dropdown
        customerSelect.addEventListener('change', function() {
            updatePO();
        });
        
        // Untuk submit final, set submit_type langsung di button
