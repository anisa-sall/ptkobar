...
<head>
  ...
  <title>Ubah Detail Surat Jalan - PT. Kobar Indonesia</title>
  <!-- plugins:css -->
...
<body>
            ...
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  ...
                  <form class="forms-sample" method="POST" action="{{ route('detailsuratjalan.update', [$suratjalan->nosuratjalan, urlencode($detail->nopart)]) }}">
                    @csrf
                    @method('PUT')
                    
                    <!-- FIELD PART -->
                    <div class="form-group">
                      <label for="part_info">Part</label>
                      <input type="text" class="form-control" id="part_info" 
                             value="{{ $detail->nopart }} - {{ $detail->part->namapart ?? $detail->nopart }}" 
                      ...
                      <div class="col-md-6">
                        <div class="form-group">
                          ...
                          <input type="number" class="form-control" id="quantity" name="quantity" 
                                 ...
                                 placeholder="0" min="1" max="{{ $max_quantity }}" required>
                    ...
                    <a href="{{ route('detailsuratjalan.index', $suratjalan->nosuratjalan) }}" class="btn btn-secondary">Cancel</a>
                  </form>
                </div>
              </div>
            </div>
  ...
  <script>
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        if (quantity <= 0) {
            e.preventDefault();
            document.getElementById('quantityError').textContent = 'Quantity harus lebih dari 0';
            quantityInput.focus();
        ...
        if (maxQuantity > 0 && quantity > maxQuantity) {
            e.preventDefault();
            document.getElementById('quantityError').textContent = `Quantity melebihi batas maksimum. Maksimum: ${maxQuantity}`;
            quantityInput.focus();
            quantityInput.style.borderColor = '#dc3545';
            return false;
        }
    ...
    document.getElementById('quantity').addEventListener('input', function() {
        ...
        if (quantity <= 0) {
            errorElement.textContent = 'Quantity harus lebih dari 0';
            this.style.borderColor = '#dc3545';
            this.style.boxShadow = '0 0 0 0.2rem rgba(220, 53, 69, 0.25)';
        } else if (maxQuantity > 0 && quantity > maxQuantity) {
            errorElement.textContent = `Quantity melebihi batas maksimum. Maksimum: ${maxQuantity}`;
            this.style.borderColor = '#dc3545';
            this.style.boxShadow = '0 0 0 0.2rem rgba(220, 53, 69, 0.25)';
        } else {
