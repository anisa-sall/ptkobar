...
<body>
        ...
        <div class="content-wrapper">
          ...
          <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Form Ubah Detail Purchase Order Baru</h4>
                  <p class="card-description">
                    Halaman Ubah Data Detail Purchase Order PT. Kobar Indonesia
                  </p>
                  <form class="forms-sample" method="POST" action="{{ route('detailpo.update', ['nopo' => $detail->nopo, 'nopart' => $detail->nopart]) }}">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="old_nopart" value="{{ $detail->nopart }}">
                    
                    <div class="form-group">
                        <label for="nopart">Part</label>
                        <select class="form-control" id="nopart" name="nopart" required>
                          <option value="" disabled hidden>Pilih Part</option>
                          @foreach ($parts as $part)
                              <option value="{{ $part->nopart }}" 
                                      data-harga="{{ $part->harga }}"
                                      {{ $part->nopart == $detail->nopart ? 'selected' : '' }}>
                                  {{ $part->namapart }}
                              </option>
                          @endforeach
                        </select>
                      ...
                      <div class="col-md-4">
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control" id="harga" name="harga" 
                                 value="{{ number_format($detail->part->harga, 0, ',', '.') }}" 
                    ...
                    <a href="{{ route('detailpo.index', $detail->nopo) }}" class="btn btn-secondary">Cancel</a>
                  </form>
                </div>
              </div>
            </div>
  ...
  <script>
    // Cek pathname dan tambahkan class active jika mengandung 'purchase-order'
    const currentPath = window.location.pathname;
    const poMenu = document.getElementById('po-menu');
    
    if (currentPath.includes('purchase-order') || currentPath.includes('detailpo')) {
        if (poMenu) {
    ...
    function calculateTotal() {
        ...
        // Set nilai input dengan format number untuk display
        hargaInput.value = formattedHarga;
        totalInput.value = formattedTotal;
        
        // Simpan nilai numerik asli di data attribute
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        // PERBAIKAN: Gunakan nilai dari data attribute yang sudah disimpan
        const hargaValue = hargaInput.getAttribute('data-numeric-value') || 0;
        const totalValue = totalInput.getAttribute('data-numeric-value') || 0;
        
        // Set nilai untuk submit (tanpa format, hanya angka)
        hargaInput.value = hargaValue;
        totalInput.value = totalValue;
        
        // Debug: Cek nilai sebelum submit
        console.log("Harga submitted:", hargaValue);
        console.log("Total submitted:", totalValue);
    });

    // Update harga dan total saat part dipilih
    document.getElementById('nopart').addEventListener('change', function() {
        calculateTotal();
    });
