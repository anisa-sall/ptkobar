...
<head>
  ...
  <title>Petugas - PT. Kobar Indonesia</title>
  <!-- plugins:css -->
            ...
            <div class="col-sm-12 grid-margin-sm stretch-card">
              <div class="card">
                <div class="card-body">                 
                  <div class="d-flex justify-content-between align-items-start mb-3">
                    <div>
                      ...
                      <p class="card-description mb-0">
                        Halaman Data Petugas PT. Kobar Indonesia
                      </p>
                    </div>
                    {{-- REMOVED: Button Tambah Petugas --}}
                  </div>
                  
                  <div class="d-flex justify-content-between align-items-end mb-3">
                    <div class="dt-length">
                      <label for="dt-length-0" class="form-label small mb-1">Show entries</label>
                      <select name="users_length" aria-controls="usersTable" class="form-select form-select-sm" id="dt-length-0" style="width: auto;" onchange="changeRecordsPerPage(this.value)">
                    ...
                    <div class="dt-search">
                      ...
                      <input type="search" class="form-control form-control-sm" id="dt-search-0" placeholder="" aria-controls="usersTable" style="width: 190px;">
                    </div>
                  </div>

                  <!-- NOTIFIKASI SUCCESS GLOBAL -->
                  @if (Session::has('success') && Session::get('success_type') === 'global')
                  ...
                  @if (Session::has('error') && Session::get('error_type') === 'global')
                  ...
                  <!-- Tampilkan info debugging kecil -->
                  <div style="display: none;" id="debug-info">
                      Count: {{ $userCount }}, Total: {{ $totalUsers }}
                  </div>

                  <div class="table-responsive">
                    <table class="table table-bordered table-hover table-sm small" id="usersTable">
                      <thead>
                        <tr>
                      ...
                      <tbody>
                          @if ($users->count() > 0)
                              ...
                              @foreach ($users as $user)
                                  <tr>
                                      <td>{{ $counter }}</td>
                                      <td class="py-1">{{ $user->name ?? 'N/A' }}</td>
                                      <td>{{ $user->email ?? 'N/A' }}</td>
                                      <td>{{ $user->departemen ?? 'N/A' }}</td>
                              ...
                              <tr>
                                  <td colspan="5" class="text-center py-3">
                                      @if($userCount === 0 && $totalUsers === 0)
                                          Tidak ada data petugas di database
                                      @else
                                          Tidak ada data petugas yang ditampilkan (mungkin karena filter)
                                      @endif
                                  </td>
                              </tr>
                          @endif
                  ...
                  @if ($users->count() > 0)
                    @foreach ($users as $user)
                      @if (Auth::id() != $user->id) {{-- Hanya tampilkan modal untuk user lain --}}
                      <div class="modal fade" id="deleteModal{{ $user->id }}" 
                          ...
                          <div class="modal-dialog modal-dialog-centered" style="max-width: 400px; margin: 0 auto;">
                              <div class="modal-content" style="max-height: 200px; overflow: hidden;">
                                  ...
                                  <div class="modal-body py-2">
                                      <p class="m-0">Yakin ingin menghapus petugas <strong>{{ $user->name }}</strong>?</p>
                                  </div>
                                  <div class="modal-footer py-2" style="border-top: 1px solid #ffffffff;">
                                      <button type="button" class="btn btn-secondary btn-sm rounded-1" data-bs-dismiss="modal">Batal</button>
                  ...
                  <nav aria-label="Page navigation example" class="mt-3">
                    <ul class="pagination justify-content-end mb-0">
                      ...
                      <li class="page-item {{ $users->currentPage() <= 1 ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $users->previousPageUrl() }}&records_per_page={{ $records_per_page }}" aria-label="Previous">
                      ...
                      @for ($i = 1; $i <= $users->lastPage(); $i++)
                        <li class="page-item {{ $users->currentPage() == $i ? 'active' : '' }}">
                          <a class="page-link" href="{{ $users->url($i) }}&records_per_page={{ $records_per_page }}">{{ $i }}</a>
                        </li>
                      @endfor
                      
                      <!-- Next Button -->
                      <li class="page-item {{ $users->currentPage() >= $users->lastPage() ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $users->nextPageUrl() }}&records_per_page={{ $records_per_page }}" aria-label="Next">
      ...
      searchInput.addEventListener('input', function() {
          ...
          if (searchTerm !== '') {
              dataRows.forEach(row => {
                  ...
                  // Cari di kolom: name (index 1), email (index 2), departemen (index 3)
                  for (let i = 1; i < 4; i++) {
                      if (cells[i]) {
              ...
              if (paginationInfo) {
                  const totalRecords = {{ $users->total() }};
                  const currentCount = {{ $users->count() }};
