              ...
              <div class="card">
                <div class="card-body">                 
                    ...
                    <table class="table table-bordered table-hover wrap-table" id="poTable">
                      ...
                      <tbody>
                        @if ($pos->count() > 0)
                          @foreach ($pos as $po)
                            <tr>
                              <td class="py-1"><strong class="text-wrap">{{ $po->nopo }}</strong></td>
                              <td class="text-wrap">{{ $po->namacustomer }}</td>
                              <td class="text-center">{{ \App\Models\PurchaseOrder::formatTanggalIndonesia($po->tglpo) }}</td>
                              <td class="text-wrap">{{ \App\Models\PurchaseOrder::formatDeliveryScheduleIndonesia($po->deliveryschedule) }}</td>
                              <td>Rp {{ number_format($po->subtotal, 0, ',', '.') }}</td>
                              <td>Rp {{ number_format($po->ppn, 0, ',', '.') }}</td>
                              <td>Rp {{ number_format($po->total, 0, ',', '.') }}</td>
                              <td class="text-wrap text-center">{{ explode(' ', $po->namapetugas)[0] }}</td>
                              <td>
                                @php
                                  $status_class = '';
                                  switch ($po->status_po) {
                              ...
                              <td class="text-center">
                                <div class="d-flex gap-1 justify-content-center">
                                  <!-- Button Detail -->
                                  <a href="{{ route('detailpo.index', urlencode($po->nopo)) }}" 
                                  ...
                                  <button type="button" 
                                    ...
                                    data-bs-target="#deletePoModal{{ preg_replace('/[^a-zA-Z0-9]/', '', $po->nopo) }}"
                                    style="padding: 0.15rem 0.4rem; font-size: 0.75rem;">
                        ...
                        @else
                          <tr>
                            <td colspan="11" class="text-center py-3">Tidak ada data purchase order</td>
                          </tr>
                        @endif
                        <!-- Row untuk pesan pencarian tidak ditemukan -->
                        <tr id="no-search-result" style="display: none;">
                          <td colspan="11" class="text-center py-3">Tidak ada data PO yang sesuai dengan pencarian</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                  ...
                  @if ($pos->count() > 0)
                    @foreach ($pos as $po)
                      @php
                        $safe_nopo = preg_replace('/[^a-zA-Z0-9]/', '', $po->nopo);
                        $modal_id = 'deletePoModal' . $safe_nopo;
                        $purchaseOrder = \App\Models\PurchaseOrder::find($po->nopo);
                        $can_delete = $purchaseOrder ? $purchaseOrder->canDelete() : false;
                      @endphp
                          ...
                          <div class="modal-content">
                            <div class="modal-header py-2" style="border-bottom: 1px solid #dee2e6;">
                              <h6 class="modal-title fs-6 m-0">
                                {{ $can_delete ? 'Konfirmasi Hapus' : 'PO Tidak Dapat Dihapus' }}
                              </h6>
                              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body py-3">
                              @if (!$can_delete)
                                <div class="text-center">
                                  ...
                                  <p class="m-0 mb-2">PO <strong>{{ $po->nopo }}</strong> tidak dapat dihapus</p>
                                  <p class="m-0 text-danger fw-bold" style="font-size: 0.7rem;">
                                    @if($purchaseOrder && $purchaseOrder->details()->exists())
                                      PO memiliki data detail atau surat jalan terkait
                                    @else
                                      PO tidak ditemukan
                                    @endif
                                  </p>
                              ...
                              @else
                                <div class="text-center">
                                  <p class="m-0 mb-2">Yakin ingin menghapus PO <strong>{{ $po->nopo }}</strong>?</p>
                                  <p class="m-0 text-danger fw-bold" style="font-size: 0.7rem;">Data PO akan dihapus permanen</p>
                                </div>
                              @endif
                            </div>
                            ...
                            @if ($can_delete)
                            <div class="modal-footer py-2" style="border-top: 1px solid #ffffffff;">
                              <form action="{{ route('po.destroy', urlencode($po->nopo)) }}" method="POST">
                                  @csrf
                                  @method('DELETE')
                  ...
                  <!-- Pagination -->
                    <nav aria-label="Page navigation example" class="mt-3">
                        <ul class="pagination justify-content-end mb-0">
                            <!-- Previous Button -->
                            <li class="page-item {{ $pos->currentPage() <= 1 ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $pos->previousPageUrl() }}&records_per_page={{ $records_per_page }}" aria-label="Previous">
                                    <span aria-hidden="true">&laquo;</span>
                                </a>
                            </li>
                            
                            <!-- Page Numbers -->
                            @for ($i = 1; $i <= $pos->lastPage(); $i++)
                                <li class="page-item {{ $pos->currentPage() == $i ? 'active' : '' }}">
                                    <a class="page-link" href="{{ $pos->url($i) }}&records_per_page={{ $records_per_page }}">{{ $i }}</a>
                                </li>
                            @endfor
                      
                      <!-- Next Button -->
                      <li class="page-item {{ $pos->currentPage() >= $pos->lastPage() ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $pos->nextPageUrl() }}&records_per_page={{ $records_per_page }}" aria-label="Next">
                    ...
                    <div class="text-muted text-left mt-2 small">
                      Menampilkan {{ $pos->count() }} dari {{ $pos->total() }} data
                    </div>
                  </nav>
                </div>
              </div>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('input', function() {
      ...
      if (searchTerm !== '') {
        ...
        if (paginationInfo) {
          const totalRecords = {{ $pos->total() }};
          const currentCount = {{ $pos->count() }};
