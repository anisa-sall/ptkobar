...
<body>
                ...
                <div class="card-body">                 
                  <div class="d-flex justify-content-between align-items-start mb-3">
                    <div>
                      ...
                      <p class="card-description mb-0">
                        Halaman Data Customer PT. Kobar Indonesia
                      </p>
                    </div>
                    <a href="{{ route('customer.create') }}" class="btn btn-primary d-flex align-items-center">
                  ...
                  <div class="d-flex justify-content-between align-items-end mb-3">
                    <div class="dt-length">
                      ...
                      <select name="customer_length" aria-controls="customerTable" class="form-select form-select-sm" id="dt-length-0" style="width: auto;" onchange="changeRecordsPerPage(this.value)">
                    ...
                    <div class="dt-search">
                      ...
                      <input type="search" class="form-control form-control-sm" id="dt-search-0" placeholder="" aria-controls="customerTable" style="width: 190px;">
                    </div>
                  </div>
                  ...
                  <div class="table-responsive">
                    <table class="table table-bordered table-hover table-sm table-xs">
                      <thead>
                        <tr>
                      ...
                      <tbody>
                        @if ($customers->count() > 0)
                          @php
                            $counter = ($customers->currentPage() - 1) * $customers->perPage() + 1;
                          @endphp
                          @foreach ($customers as $customer)
                            <tr>
                              <td>{{ $counter }}</td>
                              <td class="py-1">{{ $customer->namacustomer }}</td>
                              <td class="text-wrap">{{ $customer->alamat }}</td>
                              <td class="text-center">
                                <div class="d-flex gap-2 justify-content-center">
                                  <a href="{{ route('customer.edit', $customer->idcustomer) }}" class="btn btn-sm btn-outline-primary">
                                    <i class="bi bi-pencil-fill"></i>
                                  </a>
                                  <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deleteModal{{ $customer->idcustomer }}">
                        ...
                        <tr id="no-search-result" style="display: none;">
                          <td colspan="4" class="text-center py-3">Tidak ada data customer yang sesuai dengan pencarian</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                  ...
                  @if ($customers->count() > 0)
                    @foreach ($customers as $customer)
                      <div class="modal fade" id="deleteModal{{ $customer->idcustomer }}" 
                          tabindex="-1" 
                          ...
                          <div class="modal-dialog modal-dialog-centered" style="max-width: 400px; margin: 0 auto;">
                              <div class="modal-content" style="max-height: 200px; overflow: hidden;">
                                  ...
                                  <div class="modal-body py-2">
                                      <p class="m-0">Yakin ingin menghapus customer <strong>{{ $customer->namacustomer }}</strong>?</p>
                                  </div>
                                  <div class="modal-footer py-2" style="border-top: 1px solid #ffffffff;">
                                      <button type="button" class="btn btn-secondary btn-sm rounded-1" data-bs-dismiss="modal">Batal</button>
                  ...
                  <nav aria-label="Page navigation example" class="mt-3">
                    <ul class="pagination justify-content-end mb-0">
                      ...
                      <li class="page-item {{ $customers->currentPage() <= 1 ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $customers->previousPageUrl() }}&records_per_page={{ $records_per_page }}" aria-label="Previous">
                      ...
                      @for ($i = 1; $i <= $customers->lastPage(); $i++)
                        <li class="page-item {{ $customers->currentPage() == $i ? 'active' : '' }}">
                          <a class="page-link" href="{{ $customers->url($i) }}&records_per_page={{ $records_per_page }}">{{ $i }}</a>
                        </li>
                      @endfor
                      
                      <!-- Next Button -->
                      <li class="page-item {{ $customers->currentPage() >= $customers->lastPage() ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $customers->nextPageUrl() }}&records_per_page={{ $records_per_page }}" aria-label="Next">
                    ...
                    <!-- Info Pagination -->
                    <div class="text-muted text-left mt-2 small" data-current="{{ $customers->count() }}" data-total="{{ $customers->total() }}">
  ...
  <script>
  // Cek pathname dan tambahkan class active jika mengandung 'customer'
